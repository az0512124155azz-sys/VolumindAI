...
data class RemoteState(val connection:String="מנותק",val paired:Boolean=false,val fusionOnline:Boolean=false,val awaitingApproval:Boolean=false,val messages:List<ChatMessage> = emptyList(),val steps:List<BuildStep> = emptyList(),val screenshotUrl:String?=null,val screenshotCaption:String="עדיין אין צילום מ־Fusion",val questions:List<RemoteQuestion> = emptyList())
...
object RemoteSession {
    ...
    fun startBuild(){if(!mutable.value.fusionOnline||mutable.value.steps.isEmpty())return;socket?.send("{\"type\":\"build.start\"}");mutable.value=mutable.value.copy(awaitingApproval=false)}
    ...
    private fun receive(raw:String){runCatching{val o=json.parseToJsonElement(raw).jsonObject;when(o["type"]?.jsonPrimitive?.content){
        ...
        "build.plan"->{completionNotified=false;val titles=o["steps"]?.jsonPrimitive?.content.orEmpty().split("|").filter(String::isNotBlank);mutable.value=mutable.value.copy(steps=titles.mapIndexed{i,t->BuildStep(i,t,"waiting")},awaitingApproval=true);notify("Volumind מוכן להתחיל","תוכנית הבנייה ממתינה לאישור שלך",2101)}
        "build.step"->{val index=o["index"]?.jsonPrimitive?.content?.toIntOrNull()?:0;val status=o["status"]?.jsonPrimitive?.content?:"running";val updated=mutable.value.steps.map{if(it.index==index)it.copy(state=status)else it};mutable.value=mutable.value.copy(steps=updated,awaitingApproval=false);if(status=="error")notify("Volumind עצר","אירעה בעיה בשלב ${index+1}",2102) else if(!completionNotified&&updated.isNotEmpty()&&updated.all{it.state=="done"}){completionNotified=true;notify("המודל מוכן","Volumind סיים את כל שלבי הבנייה ב־Fusion",2103)}}
