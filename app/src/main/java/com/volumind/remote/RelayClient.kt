...
data class RemoteState(val connection:String="מנותק",val paired:Boolean=false,val messages:List<ChatMessage> = emptyList(),val steps:List<BuildStep> = emptyList(),val screenshotUrl:String?=null,val screenshotCaption:String="עדיין אין צילום מ־Fusion",val questions:List<RemoteQuestion> = emptyList())
...
object RemoteSession {
    ...
    fun open(appContext:Context,url:String,pairingCode:String){
        ...
        socket=http.newWebSocket(Request.Builder().url(url).build(),object:WebSocketListener(){
            ...
            override fun onClosed(ws:WebSocket,code:Int,reason:String){mutable.value=mutable.value.copy(connection="מנותק",paired=false)}
            override fun onFailure(ws:WebSocket,t:Throwable,response:Response?){mutable.value=mutable.value.copy(connection="שגיאת חיבור: ${t.message?:"לא ידועה"}",paired=false)}
        })
    }
    fun close(){socket?.close(1000,"service stopped");socket=null}
    fun sendChat(text:String){val clean=text.trim().take(4000);if(clean.isEmpty()||!mutable.value.paired)return;socket?.send(buildJsonObject{put("type","chat.command");put("text",clean)}.toString());mutable.value=mutable.value.copy(messages=mutable.value.messages+ChatMessage("local-${System.nanoTime()}","user",clean))}
    ...
    private fun receive(raw:String){runCatching{val o=json.parseToJsonElement(raw).jsonObject;when(o["type"]?.jsonPrimitive?.content){
        ...
        "error"->{val message=o["message"]?.jsonPrimitive?.content?:"שגיאה";mutable.value=mutable.value.copy(connection=message);notify("שגיאה ב־Volumind",message,2105)}
